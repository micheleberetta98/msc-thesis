...
\begin{enumerate}
  \item run the daemon once in the foreground in order to create the necessary files and directories;
  \item create a policy in \texttt{/var/lib/bpfcontain/policy};
  \item start the daemon in the background;
  \item run confined program by referring the policy name.
\end{enumerate}

The daemon must be run with super user privileges and the policy itself can be written in YAML.
...
\begin{itemize}
  ...
  \item an optional set of \textit{rights} to grant;
  \item an optional set of \textit{restrictions} to explicitly revoke;
  \item and an optional set of \textit{taint rules} (if no taint rules are specified, the policy
        will start tainted by default).
\end{itemize}

If the policy does not taint by default, it is strongly recommended to specify read/execute access to
the executable and red/exec-mmap access to any required external libraries.

More specifically, the rights and restrictions allows for a bigger expressive power, since it is
possible to differentiate between devices, files, folders, capabilities, network access and
inter-process communications. For example, a process might be restricted altogether from accessing
the network, and only allow communication with another specific process or policy.
...
appending anything in all of the file system, and is also tainted if it attempts to
...
A more complex example is visible in Listing \ref{lst:yaml-policy-httpd}, which shows a policy
...
\begin{code}[caption=Running WASM with BPFContain, label=lst:yaml-policy-wasm]
# Name and command of the policy
name: copy-file
...
Moreover, the restrictions and permissions applied to files and directories more specific and do not
...
There is also a notion of tainting, and there can be rules that define when to taint a process.
...
Hence, a user that is not able to use the \texttt{sudo} command is not able to make leverage of eBPF
expressive power.
This is in stark contrast with Landlock, which sets out to offer \textit{unprivileged} access control,
so that super user privileges are not required to restrict oneself.