...
it is possible to remap paths so that, for example, when mapping \texttt{/var/tmp} to \texttt{.}, every time a compiled WASM binary
accesses his ``current folder'' it will access \texttt{/var/tmp}.
...
the executable and read/exec-mmap access to any required external libraries.
...
the network, and only allow communication with another specific process.
