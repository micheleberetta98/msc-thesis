...
and then ben safely run on different platforms.
Its API provieds access to several OS-like features, such as filesystems and Berkeley sockets.
...
so that access to the filesystem must be explicitely given.
...
\begin{itemize}
  ...
  \item Environment variables are not accessible by default, but they must be explicitely declared;
  ...
  \item Not all ``classic'' funcitonality is available - when compiling C with the custom prebuilt toolchain,
  ...
  \item Fileless execution isn't feasable, since \texttt{memfd\_create} is not available due to the missing
