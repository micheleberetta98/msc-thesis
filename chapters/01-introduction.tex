...
\begin{itemize}
  \item JavaScript is a text format, while WebAssembly is a binary format, and this means that parsing WebAssembly
        is faster than parsing JavaScript;
  \item JavaScript is dynamically typed and WebAssembly is statically typed, hence code optimization for JS has to occur
        in the browser, when all JS files have been received, while optimizations for WebAssembly can occur much earlier
        in the compilation process; for example, JS has only two numeric type (a double-precision IEEE 754 value and a \textit{BigInt}
        for numbers bigger than $2^53$) but WASM can differentiate between integers and floating point numbers,
        so that when compiled the most efficient instructions can be chosen when operating on numbers;
...
The main memory layout that WebAssembly uses is a \textit{linear memory} (or simply \textit{memory}), that is represented
