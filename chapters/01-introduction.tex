...
However, even though WebAssembly strives for safety, binaries can be still exploited with both traditional attacks,
...
\begin{itemize}
  ...
  \item the \textit{directory} is the finest granularity available to the preopen action - once access is given to a specific directory, all files and subdirectories are
        ...
        writable) when using the command line runtime wrapper\footnote{It is possible to specify a basic set of permissions when using the WASI runtime library in another language, e.g. in Rust,
...
All of them, except the first, are applied to the content of a directory and not the directory itself.
\begin{itemize}
...
This last one is treated differently from the others - in order to prevent privilege escalation, it is not enough
to create a rule with that access right, but it is necessary that, when linking or renaming a file, the destination
directory hierarchy must always have the same (or a superset of) restrictions of the source hierarchy.
If that is not the case, this actions is denied with an error from the operating system.
